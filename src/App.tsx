...
import { UserProfile, DailyIntake, DailyGoals, FoodItem, DailyRecord, MedicalLog, ConsumedFood, WeightEntry } from './types';
import { Activity, BarChart2, Home, Settings, Users, Plus, Trash2, X, AlertOctagon, Scale, Bell, Lightbulb, Download } from 'lucide-react';
...
const WEIGHT_HISTORY_PREFIX = 'health_tracker_weight_';
const MEDICINES_PREFIX = 'health_tracker_medicines_';

export default function App() {
  const [activeTab, setActiveTab] = useState<'today' | 'stats' | 'weight' | 'tips' | 'medicines'>('today');
  ...
  const getBMIStatus = (bmi: number) => {
    if (bmi < 18.5) return { status: 'نحيف', color: 'text-blue-600', bg: 'bg-blue-50' };
    if (bmi < 25) return { status: 'طبيعي', color: 'text-green-600', bg: 'bg-green-50' };
    if (bmi < 30) return { status: 'وزن زائد', color: 'text-yellow-600', bg: 'bg-yellow-50' };
    return { status: 'سمنة', color: 'text-red-600', bg: 'bg-red-50' };
  };
  ...
  useEffect(() => {
    ...
    const weightKey = `${WEIGHT_HISTORY_PREFIX}${profile.id}`;

    const savedHistory = localStorage.getItem(historyKey);
    setHistory(savedHistory ? JSON.parse(savedHistory) : []);

    const savedWeightHistory = localStorage.getItem(weightKey);
  ...
  const playWarningSound = () => {
    try {
      // استخدام الصوت الطبي المتقدم من Dashboard لو موجود
      if (typeof (window as any).playMedicalWarningSound === 'function') {
        (window as any).playMedicalWarningSound();
        return;
      }
      // fallback بسيط
  ...
  const handleAddWeight = (weight: number) => {
    ...
    localStorage.setItem(`${WEIGHT_HISTORY_PREFIX}${profile.id}`, JSON.stringify(updated));
    
    // Update profile weight
    setProfiles(prev => prev.map(p => p.id === profile.id ? { ...p, weight } : p));
  };
  ...
  const handleDeleteProfile = (id: string, name: string) => {
    if(confirm(`هل أنت متأكد من حذف (${name}) وجميع بياناته وتقاريره نهائياً؟`)) {
      ...
      localStorage.removeItem(`${WEIGHT_HISTORY_PREFIX}${id}`);
      localStorage.removeItem(`${MEDICINES_PREFIX}${id}`);
  ...
  const tabs = [
    { id: 'today', label: 'اليوم', icon: Home },
    { id: 'weight', label: 'الوزن', icon: Scale },
    { id: 'stats', label: 'التقارير', icon: BarChart2 },
    { id: 'tips', label: 'نصائح', icon: Lightbulb },
    { id: 'medicines', label: 'الأدوية', icon: Bell },
  ];

  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-50 via-blue-50/30 to-emerald-50/30 font-sans text-gray-900">
      
      {/* Header */}
      <div className="bg-gradient-to-r from-emerald-900 via-emerald-800 to-teal-900 text-white py-2 px-4 text-xs sm:text-sm flex flex-col sm:flex-row justify-center sm:justify-between items-center gap-1 sm:gap-4 z-20 shadow-lg">
        <span className="font-bold tracking-wide">برمجة وتصميم: م/ مصطفى زيدان</span>
        <span dir="ltr" className="font-medium flex items-center gap-1">
          للدعم والطلبات: 
          <a href="tel:01211638551" className="font-black hover:text-emerald-200 transition-colors">01211638551</a>
        </span>
      </div>
        ...
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center justify-between w-full md:w-auto">
            <div className="flex items-center gap-3 text-emerald-600">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-black tracking-tight bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">صحتك بالدنيا</h1>
                {profile && (
                  <p className="text-xs text-gray-600 font-medium">مرحباً، {profile.name}</p>
                )}
              </div>
            </div>
            {profile && (
              <div className="md:hidden flex items-center gap-2">
                <button onClick={() => setShowProfileSwitch(true)} className="flex items-center gap-2 text-sm font-bold text-gray-700 bg-gray-100 px-3 py-1.5 rounded-xl hover:bg-gray-200 transition-all">
          ...
          <div className="flex items-center justify-between md:justify-end gap-3">
            {profile && (
              <>
                {calculateBMI() && (
                  <div className={`hidden md:flex items-center gap-2 px-4 py-2 rounded-xl ${getBMIStatus(parseFloat(calculateBMI()!)).bg} border border-gray-200`}>
                    <Scale className={`w-4 h-4 ${getBMIStatus(parseFloat(calculateBMI()!)).color}`} />
                    <div className="text-right">
                      <p className="text-xs text-gray-600 font-medium">BMI</p>
                      <p className={`text-sm font-black ${getBMIStatus(parseFloat(calculateBMI()!)).color}`}>
                        {calculateBMI()} - {getBMIStatus(parseFloat(calculateBMI()!)).status}
                      </p>
                    </div>
                  </div>
                )}
                <div className="hidden md:flex items-center gap-2">
                  <button onClick={() => setShowProfileSwitch(true)} className="flex items-center gap-2 text-sm font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-4 py-2 rounded-xl transition-all">
                  ...
                  <button onClick={exportToPDF} className="flex items-center gap-2 text-sm font-bold text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 px-4 py-2 rounded-xl transition-all shadow-md">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-6 md:py-8">
        {!profile ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
            ...
            {/* Navigation Tabs */}
            <div className="bg-white/80 backdrop-blur-lg p-1.5 rounded-2xl shadow-lg border border-gray-200 overflow-x-auto">
              <div className="flex gap-1 min-w-max">
                {tabs.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id as any)}
                    className={`flex items-center justify-center gap-2 py-3 px-4 md:px-6 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${
                      activeTab === tab.id
            ...
            {activeTab === 'weight' && (
              <WeightTracker 
                ...
                currentBMI={calculateBMI()}
                bmiStatus={calculateBMI() ? getBMIStatus(parseFloat(calculateBMI()!)) : null}
              />
            )}
            ...
            {activeTab === 'tips' && (
              <HealthTips profile={profile} intake={intake} goals={goals} />
            )}

            {activeTab === 'medicines' && (
              <MedicineReminders profileId={profile.id} />
            )}
          </div>
        )}
      </main>
      ...
      {showMedicalWarning.show && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl relative border-t-8 border-rose-600 animate-in zoom-in-95 duration-300 text-center">
      ...
      {showProfileSwitch && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl relative animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b">
              <h2 className="text-xl font-bold flex items-center gap-2"><Users className="w-5 h-5"/> الأشخاص</h2>
              <button onClick={() => setShowProfileSwitch(false)} className="bg-gray-100 p-1.5 rounded-xl text-gray-500 hover:text-red-500 transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-4 space-y-3 max-h-96 overflow-y-auto">
              {profiles.map(p => (
                <div key={p.id} className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${p.id === activeProfileId ? 'border-emerald-500 bg-emerald-50 shadow-md' : 'border-gray-100 hover:bg-gray-50 hover:border-gray-200'}`}>
                  ...
                  <button 
                    ...
                    className="text-red-400 hover:text-red-600 p-2 bg-red-50 hover:bg-red-100 rounded-xl transition-colors"
                    title="حذف هذا الشخص"
                  >
            ...
            <div className="p-4 border-t">
              <button onClick={() => { setShowProfileSwitch(false); setShowProfileEdit(true); setActiveProfileId(null); }} className="w-full bg-gradient-to-r from-gray-800 to-gray-900 text-white font-bold py-3 rounded-xl flex justify-center gap-2 hover:from-gray-900 hover:to-black transition-all shadow-md">
                <Plus className="w-5 h-5" /> إضافة شخص جديد
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Profile Edit Modal */}
      {showProfileEdit && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4 overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl relative my-auto max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b sticky top-0 bg-white z-10 rounded-t-3xl">
              <h2 className="text-xl font-bold">{profile ? 'تعديل بيانات الشخص' : 'إضافة شخص جديد'}</h2>
              <button onClick={() => { setShowProfileEdit(false); if (!profile && profiles.length > 0) setActiveProfileId(profiles[0].id); }} className="bg-gray-100 p-1.5 rounded-xl text-gray-500 hover:text-red-500 transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-5">
