        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div className="max-h-60 overflow-y-auto border border-gray-200 rounded-xl divide-y divide-gray-100">
              {/* زر السلطة السريعة ضمن القائمة */}
              {(selectedCategory === 'all' || selectedCategory === 'vegetables') && (
                <button
                  onClick={() => {
                    const veggie = foodDatabase.find(f => f.category === 'vegetables');
                    if (veggie) { onAddFood(veggie, 200); }
                    else {
                      onAddFood({ id: 'quick_salad', name: 'طبق سلطة خضراء', category: 'vegetables', protein: 2, carbs: 5, sugar: 3, calories: 35, fiber: 3 } as any, 200);
                    }
                  }}
                  className="w-full text-right px-4 py-3 hover:bg-emerald-50 transition-colors flex justify-between items-center bg-emerald-50/50 border-r-4 border-emerald-400"
                >
                  <span className="font-bold text-emerald-800 flex items-center gap-2">🥗 طبق سلطة خضراء (200 جم)</span>
                  <span className="text-xs text-emerald-600 bg-emerald-100 px-2 py-1 rounded-md font-bold">إضافة سريعة</span>
                </button>
              )}
